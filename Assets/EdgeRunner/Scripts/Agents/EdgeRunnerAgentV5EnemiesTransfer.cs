...
public class EdgeRunnerAgentV5EnemiesTransfer : Agent
{
    /*
     * EdgeRunner V5 Enemies Transfer keeps the exact V5 base vector observations:
     ...
     * No enemy observations are added so checkpoints trained with EdgeRunnerV5 can be used via --initialize-from.
    ...
    {
        if (!rewardPassedEnemies || enemyPassReward <= 0f)
        {
            return;
        }
        ...
        StompableAndroidEnemy[] stompableEnemies = FindObjectsByType<StompableAndroidEnemy>(FindObjectsInactive.Exclude);

        for (int i = 0; i < stompableEnemies.Length; i++)
        {
            StompableAndroidEnemy enemy = stompableEnemies[i];

            if (enemy != null && enemy.IsAlive && enemy.AffectsAgent)
            {
                TryRewardPassedEnemy(enemy.transform, direction);
            }
        }
    }
    ...
    private void TryRewardPassedEnemy(Transform enemyTransform, float direction)
    {
        ...
        if (passedDistance < enemyPassMargin || Mathf.Abs(deltaFromEnemy.y) > enemyPassVerticalTolerance)
        {
            return;
        }
